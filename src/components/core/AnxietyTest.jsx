import { useState } from 'react';
import { RadioGroup, FormControlLabel, Radio, Button, Typography, Box, Divider } from '@mui/material';

const questions = [
  "Feeling nervous, anxious or on edge",
  "Not being able to stop or control worrying",
  "Worrying too much about different things",
  "Trouble relaxing",
  "Being so restless that it's hard to sit still",
  "Becoming easily annoyed or irritable",
  "Feeling afraid as if something awful might happen",
  "Feeling tense or 'on edge' even in safe situations",
  "Experiencing sudden feelings of panic or impending doom without a clear cause",
  "If you checked any of the problems, how difficult have these problems made it for you to do your work, take care of things at home or get along with other people?",
];

const options = [
  { label: "Not at all", value: 1 },
  { label: "Several days", value: 0.5 },
  { label: "More than half the days", value: 0.25 },
  { label: "Nearly everyday", value: 0 }
];

const AnxietyTest = () => {
  const [answers, setAnswers] = useState(Array(questions.length).fill(null));
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
  const [score, setScore] = useState(null);

  const handleOptionChange = (value) => {
    const newAnswers = [...answers];
    newAnswers[currentQuestionIndex] = value;
    setAnswers(newAnswers);
  };

  const handleNextQuestion = () => {
    if (currentQuestionIndex < questions.length - 1) {
      setCurrentQuestionIndex(currentQuestionIndex + 1);
    } else {
      calculateScore();
    }
  };

  const calculateScore = () => {
    const totalScore = answers.reduce((sum, val) => sum + (val || 0), 0);
    setScore(totalScore);
  };

  const renderResult = () => {
    let results;
    let resultMessage;
    let severityImage = "healthy.png";

    if (score >= 7) {
      results = "Healthy";
      resultMessage = "This indicate that you are healthy and you have very few adverse symptoms and are functioning within the healthy range for the general population.";
    } else if (score >= 3.6) {
        results = "Mild";
        resultMessage = "This indicate that you have few adverse symptoms and may experience only minor limitations or interference in your daily tasks and activities.";
        severityImage = "mild.png";
    } else {
      results = "Severe";
      resultMessage = "This indicate that you may be experiencing significant psychological or physical symptoms, marked levels of interference and limitations on your daily tasks and activities.";
      severityImage = "severe.png";
    }

    return (
        <div style={{backgroundColor:'#f0f0f0', padding:'0 0 5px 0'}}>
        <div style={{ backgroundColor: "#222222", padding: "10px 0" }}>
            <Typography variant="h6" color='white' textAlign={"center"} sx={{fontWeight:'bold'}}>
                WELLNEST
            </Typography>
            <div style={{ display: "flex", justifyContent: "center" }}>
                <img src="data:image/jpeg;base64," width={'10%'} alt="" />
            </div>
            <Typography variant="h3" color="white" textAlign={"center"} sx={{fontFamily:'Itim'}}>
                Anxiety Test
            </Typography>
        </div>
        <div style={{ backgroundColor: 'white', padding: '5px 15px', borderRadius: 5, margin: '30px auto', maxWidth: '800px' }}>
        <Box sx={{textAlign:'center',py:3 }}>
        <Typography variant="h4" >Thank you for taking the assessment!</Typography>
        <Typography variant="h5">Your score is {score}/10</Typography>
        </Box>
        </div>
        <div style={{ backgroundColor: 'white', padding: '5px 15px', borderRadius: 5, margin: '30px auto', maxWidth: '800px' }}>
        <Box sx={{textAlign:'center', my: 3}}>
        <Typography variant='h4'>{results}</Typography>
        <img src={severityImage} alt="Severity Indicator" style={{ width: '200px', marginTop: '20px' }} />
        <Typography variant='h5'>Your Result : </Typography>
        <Typography variant="h6">{resultMessage}</Typography>
        </Box>
        </div>
        <div style={{ backgroundColor: 'white', padding: '5px 15px', borderRadius: 5, margin: '30px auto', maxWidth: '800px' }}>
        <Box sx={{p:3 }}>
        <Typography variant="h5" sx={{pb:3}}>What can you do next ?</Typography>
        <ul>
        <li><Typography variant="body1" sx={{pb:2}}>If you are worried about your psychological health or safety, you should immediately consult with your General Practicioner or a qualified health professional.</Typography></li>
        <li><Typography variant="body1" sx={{pb:2}}>Find out more about things you can do to help you feel better.</Typography></li>
        <li><Typography variant="body1" sx={{pb:2}}>If your worry or panic feels out of control, and especially if you feel like you may hurt yourself, then talking to someone is really important. Get immediate assistance from the Depression Helpline 0800 111 757.</Typography></li>
        <li><Typography variant="body1">Make sure you have positivity around you and try making yourself happy all the time.</Typography></li>
        </ul>
        </Box>
        </div>
      </div>
    );
  };

  if (score !== null) {
    return renderResult();
  }

  return (
    <div style={{backgroundColor:'#f0f0f0', padding:'0 0 5px 0'}}>
    <div style={{ backgroundColor: "#222222", padding: "10px 0" }}>
        <Typography variant="h6" color='white' textAlign={"center"} sx={{fontWeight:'bold'}}>
            WELLNEST
        </Typography>
        <div style={{ display: "flex", justifyContent: "center" }}>
            <img src="data:image/jpeg;base64," width={'10%'} alt="" />
        </div>
        <Typography variant="h3" color="white" textAlign={"center"} sx={{fontFamily:'Itim'}}>
            Anxiety Test
        </Typography>
    </div>
    <div style={{ backgroundColor: 'white', padding: '5px 15px', borderRadius: 5, margin: '15px auto', maxWidth: '600px' }}>
      <Box sx={{ p: { xs: 2, sm: 3, md: 4 } }}>
        <Typography variant="h6">
          Question {currentQuestionIndex + 1} of {questions.length}
        </Typography>
        <Typography variant="h5" sx={{ mt: 2 }}>
          {questions[currentQuestionIndex]}
        </Typography>
        <RadioGroup
            value={answers[currentQuestionIndex]}
            onChange={(e) => handleOptionChange(parseFloat(e.target.value))}
            sx={{ mt: 2 }}
          >
            {options.map((option, index) => (
              <Box
                key={option.label}
                sx={{
                  '&:hover': { backgroundColor: '#0001' },
                  borderRadius: 1,
                  padding: '5px 10px'
                }}
              >
                <FormControlLabel
                  value={option.value}
                  control={<Radio />}
                  label={option.label}
                />
                {index < options.length - 1 && <Divider />}
              </Box>
            ))}
        </RadioGroup>
        <Button
          variant="contained"
          color="primary"
          onClick={handleNextQuestion}
          sx={{ mt: 3, width: '100%' }}
          disabled={answers[currentQuestionIndex] === null}
        >
          {currentQuestionIndex < questions.length - 1 ? "Next" : "Submit"}
        </Button>
      </Box>
    </div>
    </div>
  );
};

export default AnxietyTest;
